...
d_plot <- d_long |> 
  group_by(r_time) |> 
  summarize(hdd_mean = mean(hdd), hdd_sd = sd(hdd)) |> 
  mutate(study_id = 999)

d_plot <- d_plot |> 
  mutate(hdd_pred = predict(m_raw, newdata = d_plot, allow.new.levels = TRUE)) |> 
  glimpse()

d_plot |> 
  ggplot(aes(x = r_time)) +
    geom_point(aes(y = hdd_mean)) +
    geom_line(aes(y = hdd_pred), color = "red", linewidth = 2) 
```
